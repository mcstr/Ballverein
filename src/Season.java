...
public class Season {
    ...
    public void configuration() {
        ...
        if (!new File("Season", this.teamFileName).exists()) {
            ...
            EditSeasonFile.saveSeasonFile(this.teamsList, this.teamFileName);
            EditFixtureFile.saveFixturesFile(this.fixturesList, this.fixtureFileName);
        } else {
            ...
            Console.printNumberOfTeams(this.teamsList.size());
        }
            ...
            if (this.teamsList.size() != 0) {
                ...
                for (int i = 1; i < (n + 1); i++) {
                    ...
                    for (int k = 1; k < (n + 1) / 2; k++) {
                        ...
                        if (k % 2 != 0) {
                            counter++;
                            games.add(new Game(this.teamsList.get(teamA - 1), this.teamsList.get(teamB - 1), fixture.id,
                                    counter));
                        } else {
                            counter++;
                            games.add(new Game(this.teamsList.get(teamB - 1), this.teamsList.get(teamA - 1), fixture.id,
                                    counter));
                        }
                    }
                    fixture.games = games;
    ...
    public void registerGameResults() {
        if (this.fixturesList.size() > 0) {
            ...
            if (game.played == true) {
                Console.printGameAlreadyPlayed();
                char option = this.scanner.next().charAt(0);
    ...
    public void showTable() {
        if (this.teamsList.size() != 0) {
            this.teamsList.sort(Comparator.comparing(Team::getPoints).thenComparing(Team::getScoredGoals)
                    .thenComparing(Team::getRecievedGoals).reversed());
